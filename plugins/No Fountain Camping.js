...
function onMapStart() {
	...
	for(i=0; i<fountains.length; i++) {
		...
		dota.addNewModifier(fountain, truestrike, "modifier_item_monkey_king_bar", "item_monkey_king_bar", {});
		
		// Give this fountain ursa's fury swipes
		var swipes = dota.createAbility(fountain, 'ursa_fury_swipes');
		swipes.netprops.m_iLevel = 4;
		dota.addNewModifier(fountain, swipes, "modifier_ursa_fury_swipes", "ursa_fury_swipes", {});
		...
		dota.addNewModifier(fountain, bash, "modifier_spirit_breaker_greater_bash", "spirit_breaker_greater_bash", {});
	}
